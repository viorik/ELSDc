...
static double poly_nfa( PImageDouble angles, PImageInt used, PolyRect *poly, 
                        ...
                        int *size_buff, double logNT_poly, FILE *fdebug )
{
  int pts;
  double err;
                            ...
                            Point **tmp_buff, double logNT_poly, FILE *fdebug )
{
  ...
  fprintf(fdebug,"poly_improve %f %f\n",poly->wmax,poly->wmin);
  /* check parameters */
  if( (angles == NULL) || (angles->data == NULL) )
  ...
  *size_local_buff = 0;
  ...
  log_nfa = poly_nfa( angles, used, poly, wmin, wmax, *local_buff, 
                      size_local_buff, logNT_poly, fdebug );
      ...
      while( wmax - delta > 0.0)
        {
          ...
          log_nfa_new = poly_nfa( angles, used, poly, wmin, wmax, *tmp_buff, 
                                  &size_tmp_buff, logNT_poly, fdebug);
...
{    
  ...
  int size_tmp_buff;
  fprintf(fdebug,"circ_ring_improve %f %f\n", cring->wmax, cring->wmin);
  /* check parameters */
  if( (angles == NULL) || (angles->data == NULL) )
...
{ 
  ...
  fprintf(fdebug,"ell_ring_improve %f %f \n", ering->wmax, ering->wmin );
  /* check parameters */
  if( (angles == NULL) || (angles->data == NULL) )
                   ...
                   int *poly_count_max, int **labels, int lbl, FILE *fdebug )
{
  ...
  if( poly_count>*poly_count_max )
    {
      *poly_out = (Polygon *) realloc ( *poly_out, (2*poly_count) *
                                                   sizeof(Polygon) );
      if( poly_out==NULL ) fprintf(fdebug,"null\n");
  ...
  fprintf(fdebug,"end add_poly_out %d\n", (*poly_out)[poly_count-1].dim);
}
  ...
  if( ell_count>*ell_count_max )
    {
      *ell_out = ( Ring *) realloc ( *ell_out, (2*ell_count) * sizeof(Ring) );
      if( *ell_out==NULL ) fprintf(fdebug,"null\n");
  ...
  for( ; list_p; list_p = list_p->next )
    { 
      ...
      if( !curve_grow( gradmag, angles, used, reg, &reg_size, density_th,
                       prec, poly, &label, pext2, pext1, &spir, fdebug ) ) 
        continue;
      ...
      if( get_ring( reg, reg_size, ang0, cparam, pext1, pext2, CIRCLE,
        ...
        {
          fprintf(fdebug,"CC\n");
          fprintf(fdebug,"%f %f %f %f %f %f %f\n",cparam[0],cparam[1],cparam[2],cparam[3],cparam[4],cring.ang_start, cring.ang_end);
      ...
      if( get_ring( reg, reg_size, ang0, eparam, pext1, pext2, ELLIPSE,
        ...
        {
          fprintf(fdebug,"EE\n");
          fprintf(fdebug,"%f %f %f %f %f %f %f\n",eparam[0],eparam[1],eparam[2],eparam[3],eparam[4],ering.ang_start,ering.ang_end);
      ...
      if( best_type == 'c' )
        {
          ...
          fprintf(fdebug,"%f %f %f %f %f %f %f\n",rr->cx,rr->cy,rr->ax,rr->bx,rr->theta,rr->ang_start,rr->ang_end);
          /* free(rr); */
        }
      else if( best_type == 'p' ) 
        {
          ...
          fprintf( fdebug, "%d %d\n",*poly_count, (*poly_out)[*poly_count-1].dim );
          for( i=0; i<(*poly_out)[*poly_count-1].dim; i++ )
            fprintf( fdebug,"%f %f ", (*poly_out)[*poly_count-1].pts[i].x, (*poly_out)[*poly_count-1].pts[i].y);
          fprintf( fdebug, "\n" );
        }
